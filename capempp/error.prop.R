error.prop <- function (data.obj, pairscan.obj, perm = FALSE, verbose = FALSE, run.parallel = TRUE, n.cores = 4, just.m = FALSE) {
	...
	#====================================================================================
		...
		get.pair.coeffs <- function(marker.pair.number){
			...
			if(length(non.zero) > 0 && length(na.locale) == 0){
				...
				if(!just.m){
					...
					inf.coeffs <- calc.m(markers = marker.mat[marker.pair.number,], beta.m = beta.main, se = beta.se, beta.cov)	
					}
				}else{
				inf.coeffs <- NULL	
				}
		...
		get.multi.pair.coeffs <- function(marker.pair.number.v){
		  result <- do.call("rbind", lapply(marker.pair.number.v, function(x) get.pair.coeffs(x)))
		  return(result)
		}
	...
	if(perm){
		...
		n.perm <- nrow(pairscan.obj$pairscan.perm[[1]][[1]])
	...
	if (run.parallel) {
	  ...
	  influence.coeffs <- foreach(p = c(1:length(chunked.pairs)), .combine = "rbind", .export = c("calc.delta.errors", "pseudoinverse", "propagate", "calc.m")) %dopar% {
	    get.multi.pair.coeffs(chunked.pairs[[p]])
	  }				
	  stopCluster(cl)
	  ...
	  for (p in index) {
	    influence.coeffs <- rbind(influence.coeffs, get.multi.pair.coeffs(chunked.pairs[[p]]))
	  }
