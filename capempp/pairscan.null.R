	...
	while(final.perm < pairscan.null.size){
		...
		if(marker.selection.method != "by.gene" && marker.selection.method != "from.list"){
			...
			for(p in 1:num.pheno){ 
				...
				one.singlescan.tstats <- one.singlescanDO(phenotype.vector = pheno[perm.order,p], genotype.mat = geno, ref.allele = ref.allele, model.family = model.family, run.parallel = run.parallel, n.cores = n.cores)
				...
				perm.data.obj <- select.markers.for.pairscan(data.obj, singlescan.obj = single.scan.result, geno.obj, num.alleles = n.top.markers, peak.density = data.obj$peak.density, window.size = data.obj$window.size, tolerance = data.obj$tolerance, plot.peaks = FALSE, verbose = TRUE)
				}
			...
			#SNPs near genes
			perm.data.obj <- select.markers.for.pairscan.by.gene(data.obj, ref.allele = ref.allele, geno.obj = geno.obj, gene.list = sample(gene.list), num.snps = ncol(data.obj$geno.for.pairscan), organism = organism)
			}
			...
			for(p in 1:num.pheno){
				...
				pairscan.results <- one.pairscan.parallel(perm.data.obj, phenotype.vector = pheno[perm.order,p], genotype.matrix = perm.data.obj$geno.for.pairscan, paired.markers = top.marker.pairs, n.perm = 0, run.parallel = run.parallel, n.cores = n.cores, verbose = FALSE)
