...
one.pairscan.parallel <- function(data.obj, phenotype.vector, genotype.matrix, int = NULL, covar.vector = NULL, paired.markers, n.perm = 0, run.parallel = TRUE, verbose = FALSE, n.cores = 4){
		...
		is.char <- as.logical(is.na(suppressWarnings(as.numeric(colnames(covar.vector)[1]))))
		num.markers <- dim(genotype.matrix)[2]
		covar.names <- get.covar(data.obj)$covar.names
		#============================================================================
		...
		if (run.parallel) {
		  ...
		  pair.results.list <- foreach(m = 1:length(chunked.pairs), .export = c("phenotype.vector", "rankMatrix")) %dopar% {
		...
		if(n.perm > 0){
			...
			if (run.parallel) {
			  ...
			  cl <- makeCluster(n.cores)
			  registerDoParallel(cl)
			  perm.results <- foreach(p = 1:n.perm, .export = c("phenotype.vector", "rankMatrix")) %dopar% {
			    one.perm(p)
			  }
			  stopCluster(cl)
